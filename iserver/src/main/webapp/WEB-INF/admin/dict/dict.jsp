	...
	<div data-options="region:'center',title:'数据字典列表',iconCls:'icon-ok'">
		<table id="dictid"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'code',align:'center'" width="150">标识</th>
		...
		<div id="toolbar" style="padding: 5px; height: auto">
			<div style="padding: 5px;">
				<font>数据字典名称：</font> <input id="dn" name="name"
					class="easyui-validatebox" type="text" style="width: 200px;" /> <a
					href="javascript:void(0);" class="easyui-linkbutton"
					iconCls="icon-search" onclick="findDict()">查询</a>
			</div>
			<div>
			...
			<div data-options="region:'center'" style="padding: 10px;">
				<form id="form" method="post">
					<table align="center" width="100%">
						<tr>
							<td>字典名称：</td>
							<td><input id="name" name="dictName"
...
function saveDict(){
	var name = $("#name").val();
	if(name == ""){
		$.messager.alert("系统提示","请填写数据字典名称","warning");
		return;
	}
	var code = $("#code").val();
	if(code == ""){
		$.messager.alert("系统提示","请填写数据字典代码","warning");
		return;
	}
	var options ={
			url:"${app}/admin/json/saveDict",
...
function deleteDict(){
	...
	$.messager.confirm('系统提示', '确定要删除吗?', function(r){
		if (r){
			$.getJSON("${app}/admin/json/deleteDict?id="+record.id,function(data){
				if(checkData(data)){
					findDict();
				}
			});			
...
function findItem(){
	...
	window.location.href="${app}/admin/dictitem?dictId="+record.id;
}
...
function findDict(){
	var name = $("#dn").val();
	 $('#dictid').datagrid({
	        ...
	        fit: true,//自动大小  
	        url:'${app}/admin/json/findDictList?name='+name,  
	        remoteSort:false, 
	        idField:'id',  
