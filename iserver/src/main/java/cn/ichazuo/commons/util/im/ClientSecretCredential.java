...
public class ClientSecretCredential extends Credential {
	...
	public ClientSecretCredential(String clientID, String clientSecret, String appKey) {
		super(clientID, clientSecret);
		CLIENT_TOKEN_TARGET = EndPoints.TOKEN_APP_TARGET.resolveTemplate("org_name", appKey.split("#")[0])
			...
			try {
				...
				if (null != tokenRequest.get("error")) {
					throw new RuntimeException(
							"Some errors occurred while fetching a token by " + "grant_type[" + GRANT_TYPE
									+ "] client_id[" + tokenKey1 + "]" + " and client_secret[" + tokenKey2 + "] .");
				}
				...
				throw new RuntimeException("Some errors occurred while fetching a token by " + "grant_type["
						+ GRANT_TYPE + "] client_id[" + tokenKey1 + "]" + " and client_secret[" + tokenKey2 + "] .");
			}
		}
