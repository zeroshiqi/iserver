...
public class CrowdfundingController extends BaseController {
	...
	public JSONObject checkCrowdfunding(String unionId, Long courseId) {
		try {
			...
			//根据UnionId和课程Id判断是否购买了课程
			Integer status = orderService.findWebOrderByUnionId(courseId, unionId);
			if(!NumberUtils.isNullOrZero(status)){
				//查询线下课程订单审核状态
				String reviewStatus = orderService.findWebOrderReviewStatus(courseId, unionId);
				String orderCode = orderService.findWebOrderCode(courseId, unionId);
				return ok2(reviewStatus,"have1",orderCode,course.getBeginTime());
			}
...
//				}
	 ...
	 */
	@ResponseBody
	@RequestMapping("/checkCrowdfundingNew")
	public JSONObject checkCrowdfundingNew(String unionId, Long courseId) {
		try {
			if (StringUtils.isNullOrEmpty(unionId) || NumberUtils.isNullOrZero(courseId)) {
				return error("参数错误");
			}
