...
import cn.ichazuo.controller.app.ChuanglanSMS;
...
//		}
	...
	/**
	 * @Title: sendCLMessage
	 * @Description: (发送创蓝验证码短信)
	 * @param mobile
	 *            手机号
	 * @throws UnsupportedEncodingException
	 */
	public void sendCLMessage(String mobile) throws UnsupportedEncodingException {
		String code = CodeUtils.getCode(4);
		codeService.saveCode(mobile, code);
		// 调用发送短信接口
		// String username = configInfo.getDuanxinbaoUserName();// 短信宝帐户名
		...
		String content = java.net.URLEncoder.encode(CODE_CONTENT.replace("#code#", code), "UTF-8");// 发送内容
		content.replaceAll(" ", "");
		sendCLMsg(mobile, CODE_CONTENT.replace("#code#", code));
		// HttpSend send = new HttpSend(
		// "http://www.smsbao.com/sms?u=" + username + "&p=" + pass + "&m=" +
		// phone + "&c=" + content);
		// try {
...
//		}
	 ...
	 * 发送创蓝短信
	 */
	public void sendCLMsg(String mobile, String content){
		String url = "https://sms.253.com/msg/send";// 应用地址
		String un = "N3606610";// 账号
		String pw = "65e67e31";// 密码
		String rd = "1";// 是否需要状态报告，需要1，不需要0
		String ex = null;// 扩展码
		try {
			String returnString = HttpSender.batchSend(url, un, pw, mobile, content, rd, ex);
...
//		response = client.sendMessage("15910491294",str.replace("#xxx#", r.getStr("nick_name"))+text.replaceAll(" ",""));
	...
	public void pushIOSMessage(ClientEnum client, String alert, String alias) {
		...
		PushPayload payload = PushPayload.newBuilder().setPlatform(Platform.ios()).setAudience(Audience.alias(alias))
				.setNotification(Notification.newBuilder()
						.addPlatformNotification(IosNotification.newBuilder().setAlert(alert).setBadge(0)
								.setSound("happy").addExtra("type", "1").build())
						.build())
	 ...
	 public void pushAndroidMessage(ClientEnum client, String alert, String alias) {
			JPushClient jpushClient = getPushClient(client);
			PushPayload payload = PushPayload.newBuilder()
	                .setPlatform(Platform.android())
	                .setAudience(Audience.alias(alias))
//	                .setAudience(Audience.tag(alias))
	                .setNotification(Notification.android(alert, "", null))
	                .build();
//			PushPayload payload = PushPayload.newBuilder()
//            .setPlatform(Platform.android_ios())
//            .setAudience(Audience.newBuilder()
//                    .addAudienceTarget(AudienceTarget.tag(alias))
...
//			return new JPushClient(configInfo.getJpushChaZuoSecret(), configInfo.getJpushChaZuoAppKey(), 3);
			//好多课参数
			return new JPushClient("bb70f349569760bb3ea0af4d", "cdda5693ca8eee98d428b4e2", 3);
		} else {
...
//	@SuppressWarnings("unchecked")
//	public String findAllQuestionByType(int type) {
//		List<Question> list = null;
//		String questionKey = "questionKey--code--1";
//		if (redisClient.isExist(questionKey)) {
//			list = (ArrayList<Question>) redisClient.getObject(questionKey);
...
//	}
	...
	public UserInfo findUserEmailInfoByUnionId(String unionId){
		return commonDao.findUserEmailInfoByUnionId(unionId);
	}
	//根据unionId 查询发票信息
	public UserInfo findInvoiceDetailByUnionId(String unionId){
		return commonDao.findInvoiceDetailByUnionId(unionId);
	}
	//根据orderNo 查询发票信息
	public UserInfo findInvoiceDetailByOrderNo(String orderNo){
		return commonDao.findInvoiceDetailByOrderNo(orderNo);
	}
	
	//根据unionId 查询根据订单编号查询赠送信息
	public GiftInfo findGiftDetailByOrderNo(String orderNo){
		return commonDao.findGiftDetailByOrderNo(orderNo);
	}
