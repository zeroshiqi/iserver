...
<script type="text/javascript" charset="utf-8"
	src="${app}/ueditor/umeditor.js"> </script>
<script type="text/javascript" charset="utf-8"
		...
		<table id="courseid"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'lon' , hidden:'true'" width="40">lon</th>
					<th data-options="field:'lat' , hidden:'true'" width="40">lat</th>
					...
					<th data-options="field:'beginTime' , hidden:'true'" width="40">beginTime</th>
					<th data-options="field:'cover' , hidden:'true'" width="40">cover</th>
					<th data-options="field:'avatar' , hidden:'true'" width="40">avatar</th>
					<th data-options="field:'cityId' , hidden:'true'" width="40">cityId</th>
					<th data-options="field:'teacherId' , hidden:'true'" width="40">teacherId</th>
					<th data-options="field:'courseContent' , hidden:'true'" width="40">courseContent</th>
					<th data-options="field:'offlineTypeId' , hidden:'true'" width="40">offlineTypeId</th>
					<th data-options="field:'courseName' , align:'center'" width="80">课程名称</th>
					<th data-options="field:'typeStr' , align:'center'" width="100">课程类型</th>
					<th data-options="field:'price' , align:'center'" width="40">价格</th>
					<th data-options="field:'courseTime' , align:'center'" width="80">上课时间</th>
					<th data-options="field:'cityName' , align:'center'" width="30">城市名称</th>
					<th data-options="field:'teacherNames' , align:'center'"
						width="100">讲师</th>
					<th data-options="field:'studentNum' , align:'center'" width="50">招生总数</th>
					<th data-options="field:'address' , align:'center'" width="120">地址</th>
					<th data-options="field:'commentCount' , align:'center'" width="40">评论数</th>
					<th data-options="field:'star' , align:'center'" width="30">评分</th>
					<th data-options="field:'favourCount' , align:'center'" width="30">点赞数</th>
				</tr>
			</thead>
		</table>
...
$("#courseType").combobox({
	onChange:function(newValue,oldValue){
		if(newValue == 1){
			window.location.href="${app}/admin/online";
		}
    }
});
...
$(function(){
	$("#courseType").combobox('setValue',0);
	findCourse();
	$("#container").hide();
});
//修改时
function editRecord(){
	...
	ue.setContent(record.courseContent);
	$("#courseTime").val(record.courseTime);
	$("#lon").val(record.lon);
	$("#lat").val(record.lat);
	$("#teacherNames").val(record.teacherNames);
	$("#teacherId").val(record.teacherId);
	$("#price").val(record.price);
	$("#studentNum").val(record.studentNum);
	$("#courseType1").combobox('setValue',record.type);
...
function findCourse(){
	var type = $("#courseType").combobox("getValue");
	 $('#courseid').datagrid({
	        ...
	        fit: true,//自动大小  
	        url:'${app}/admin/json/findCourseList?type=0',  
	        remoteSort:false, 
	        idField:'id',  
...
function deleteCourse(){
	...
	$.messager.confirm('系统提示', '确定要删除吗?', function(r){
		if (r){
			$.getJSON("${app}/admin/json/deleteCourse?id="+record.id+'&type=0',function(data){
				if(checkData(data)){
					findCourse();
				}
			});			
...
function showimageList(){
	...
	window.location.href="${app}/admin/courseImage?courseId="+record.id;
}
</script>
</html>